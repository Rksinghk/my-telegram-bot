...
# Channel Usernames (Verify karne ke liye)
    ...
    if not await check_join(user_id):
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                ...
                InlineKeyboardButton(text="➜ Join 2", url="https://t.me/earn_dailyrewards")
            ],
...
async def bonus(message: types.Message):
    ...
    if data and (now - data[0] < 86400):
        await message.answer("⏳ Come back after 24 hours for daily bonus.")
    else:
...
async def get_amount(message: types.Message, state: FSMContext):
    ...
    await message.answer("🏦 Please send your UPI ID:")
